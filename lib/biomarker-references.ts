...
// Índice das chaves canônicas já normalizadas (ex: "ast-tgo" → "ast tgo").
const NORMALIZED_KEYS: Record<string, string> = Object.keys(REFERENCES).reduce(
  (acc, key) => { acc[normalizeText(key)] = key; return acc; },
  {} as Record<string, string>,
);

// Nomes descritivos de laudo que não viram slug canônico (ex: "GAMA GLUTAMIL
// TRANSFERASE" → ggt). Casados por substring no texto normalizado de slug + nome.
...
export function getBiomarkerInfo(slug?: string | null, name?: string | null): string | null {
  ...
  // 3. Fallback por palavra-chave no texto combinado (slug + nome)
