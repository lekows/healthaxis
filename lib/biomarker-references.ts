...
const REFERENCES: Record<string, SexRanges> = {
  ...
  "taxa-filtracao-glomerular": { universal: { min: 90 }, descricao: "Taxa de filtração glomerular estimada (eTFG/eGFR), geralmente por CKD-EPI, em mL/min/1,73m². Estima a capacidade de filtração dos rins; valores abaixo de 60 mantidos por 3 meses sugerem redução da função renal (doença renal crônica)." },
  ...
  "calcio-ionico": { universal: { min: 1.12, max: 1.32 }, descricao: "Fração livre (ionizada) do cálcio no sangue, em mmol/L — a forma biologicamente ativa. Menos influenciada pela albumina que o cálcio total; útil em pacientes críticos e distúrbios ácido-base." },
  ...
  "base-excess-venoso": { universal: { min: -2, max: 3 }, descricao: "Excesso de base (B.E.) da gasometria venosa. Componente metabólico do equilíbrio ácido-base medido em sangue venoso; costuma ser levemente mais alto que o arterial e não deve ser comparado diretamente com o valor arterial." },
  "ph-arterial": { universal: { min: 7.35, max: 7.45 }, descricao: "pH do sangue arterial. Reflete o equilíbrio ácido-base sistêmico; abaixo da faixa indica acidemia e acima, alcalemia. Interpretar junto com PaCO₂ e bicarbonato." },
  "ph-venoso": { universal: { min: 7.31, max: 7.41 }, descricao: "pH do sangue venoso. Costuma ser um pouco menor que o arterial; avalia o equilíbrio ácido-base quando não há gasometria arterial disponível e não deve ser comparado diretamente com o pH arterial." },
  "hco3-arterial": { universal: { min: 22, max: 26 }, descricao: "Bicarbonato (HCO₃⁻) da gasometria arterial. Principal componente metabólico do equilíbrio ácido-base; baixo sugere acidose metabólica e alto, alcalose metabólica ou compensação respiratória." },
  "hco3-venoso": { universal: { min: 22, max: 29 }, descricao: "Bicarbonato (HCO₃⁻) da gasometria venosa. Componente metabólico do equilíbrio ácido-base medido em sangue venoso; interpretar conforme o tipo de amostra e o contexto clínico." },
  lactato: { universal: { min: 0.5, max: 2.2 }, descricao: "Lactato sanguíneo (mmol/L; alguns laboratórios reportam em mg/dL). Elevado indica hipoperfusão/metabolismo anaeróbio (ex.: sepse, choque, exercício intenso). A faixa depende de a amostra ser venosa ou arterial e da unidade — confira sempre a referência do laudo." },
  ...
  "atividade-protrombina": { universal: { min: 70, max: 100 }, descricao: "Atividade de Protrombina (%). Expressa o TP como percentual da atividade normal; valores baixos indicam prolongamento do TP (risco de sangramento ou uso de anticoagulante oral)." },
  "protrombina-relacao": { universal: { min: 0.8, max: 1.25 }, descricao: "Relação do Tempo de Protrombina (paciente/controle). Base do cálculo do INR; valores acima da faixa indicam prolongamento do TP." },
  ...
  "ttpa-relacao":      { universal: { min: 0.8, max: 1.2 }, descricao: "Relação do TTPA (paciente/controle). Padroniza o resultado do TTPA; valores acima da faixa indicam prolongamento (deficiência de fator ou uso de heparina)." },
  ...
  "nt-probnp": { descricao: "NT-proBNP (fragmento N-terminal do pró-peptídeo natriurético tipo B). Marcador de sobrecarga/estresse cardíaco usado para avaliar insuficiência cardíaca; os pontos de corte dependem fortemente da idade e da função renal e devem ser interpretados junto com o quadro clínico." },
...
const KEYWORD_ALIASES: [RegExp, string][] = [
  ...
  [/atividade.*protrombina/, "atividade-protrombina"],
  [/rela[cç].*protrombina|protrombina.*rela[cç]/, "protrombina-relacao"],
  [/rela[cç].*(ttpa|aptt)|(ttpa|aptt).*rela[cç]/, "ttpa-relacao"],
  [/protrombina|\btp\b/, "tempo-protrombina"],
  [/\binr\b/, "inr"],
  [/tromboplastina.*parcial|\bttpa\b|\baptt\b/, "ttpa"],
  [/nt.?pro.?bnp|pro.?bnp/, "nt-probnp"],
  [/lactato|acido latico/, "lactato"],
  [/filtracao glomerular|\begfr\b|\btfg\b|\betfg\b|ckd epi/, "taxa-filtracao-glomerular"],
  [/calcio ionico|calcio ionizado/, "calcio-ionico"],
