...
-- Tipos espelham exatamente o banco de produção.

-- Estado da extração: uploaded | processing | processed | error.
-- Distinta de documents.status (reviewed | pending | processing), que reflete a curadoria do laudo.
alter table public.documents add column if not exists extraction_status text not null default 'uploaded';

-- Progresso da extração de 0 a 100, consumido pela barra de progresso no upload.
alter table public.documents add column if not exists extraction_progress integer not null default 0;
...
alter table public.documents add column if not exists extracted_at timestamp with time zone;

comment on column public.documents.extraction_status   is 'Pipeline de extração de documentos: uploaded | processing | processed | error.';
